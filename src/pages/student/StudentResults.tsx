...
export const StudentResults: React.FC = () => {
  ...
  const history = studentAssessmentResults || [];
  const currentResult: StudentAssessmentResult = history[selectedResultIndex] || {
    id: 'ASM-LATEST',
    assessmentName: 'Python Self Assessment',
    studentId: studentProfile?.id || 'STU-001',
    studentName: studentProfile?.name || 'Student Candidate',
    studentEmail: studentProfile?.email || 'student@careerflow.ai',
    ...
    status: 'Evaluated',
    questionAnswers: [],
    strengths: ['Python Core Principles', 'Algorithm Analysis', 'Clean Code'],
    ...
    <div className="space-y-8 max-w-5xl mx-auto animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-5">
        ...
        <div className="bg-white rounded-2xl p-5 border border-[#E2E8F0] shadow-xs">
          ...
          <span className="text-[11px] text-slate-500">Recorded in Database</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-[#E2E8F0] shadow-xs space-y-4">
        ...
        {history.length === 0 ? (
          <div className="text-center py-8 text-slate-400 text-xs">
            No assessment attempts found in database. Take your first test from the Assessment Center.
          </div>
        ) : (
