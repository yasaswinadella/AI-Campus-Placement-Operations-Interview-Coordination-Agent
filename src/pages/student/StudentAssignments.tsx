        ...
        <div className="space-y-4">
          {myPendingAssignments.length === 0 ? (
            <div className="space-y-6">
              <div className="p-6 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 rounded-2xl border border-indigo-500/20 text-white shadow-xl space-y-3">
                <div className="flex items-center gap-2">
                  <Sparkles className="w-5 h-5 text-indigo-400" />
                  <h3 className="text-base font-bold text-white">
                    Standardized 50-Question Placement Benchmark Suites Ready
                  </h3>
                </div>
                <p className="text-xs text-slate-300 max-w-2xl leading-relaxed">
                  Select any domain below to launch a comprehensive 50-question, 60-minute proctored placement benchmark. Your score and skill analytics will be verified and stored in your profile.
                </p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {STANDARDIZED_50Q_ASSESSMENTS.map((asst) => (
                  <div
                    key={asst.id}
                  ...
                  >
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="px-2.5 py-0.5 rounded-full text-[11px] font-extrabold bg-indigo-50 text-[#4F46E5] border border-indigo-100">
                          {asst.skill}
                        </span>
                      ...
                      <div>
                        <h4 className="font-bold text-sm text-[#0F172A]">{asst.name}</h4>
                        <p className="text-[11px] text-slate-500 mt-1">
                          Comprehensive 50-question placement evaluation with instant grading.
                        </p>
                      </div>
                      ...
                      <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 grid grid-cols-3 gap-2 text-center text-xs">
                        <div>
                          ...
                          <span className="font-bold text-slate-800">50 Qs</span>
                        </div>
                        <div>
                          <span className="text-slate-400 text-[10px] block font-semibold">Duration</span>
                          <span className="font-bold text-indigo-600">60 mins</span>
                        </div>
                        <div>
                          <span className="text-slate-400 text-[10px] block font-semibold">Total Marks</span>
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const directAsg: StudentAssignment = {
                          id: `ASG-${asst.id}-${Date.now()}`,
                          assessmentId: asst.id,
                          assessmentName: asst.name,
                          skill: asst.skill,
                          difficulty: asst.difficulty,
                          totalQuestions: 50,
                          mcqCount: 50,
                          codingCount: 0,
                          descriptiveCount: 0,
                        ...
                        handleStartAssessment(directAsg);
                      }}
                      className="w-full py-2.5 bg-[#4F46E5] hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-xs transition-all flex items-center justify-center gap-2 active:scale-98 cursor-pointer"
                    >
                      <Play className="w-3.5 h-3.5 fill-current" />
                      <span>Start 50-Q Benchmark</span>
                    </button>
                  </div>
                ))}
              </div>
