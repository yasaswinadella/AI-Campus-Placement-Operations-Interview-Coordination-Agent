...
export const StudentApplications: React.FC = () => {
  ...
  const myApplications = applications.filter((a) => a.studentId === studentProfile.id);
    ...
    <div className="space-y-8 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="px-4 py-2 bg-[#4F46E5] hover:bg-indigo-700 text-white font-semibold text-xs rounded-xl shadow-xs transition-all flex items-center gap-1.5"
        >
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        {['ALL', 'APPLIED', 'SHORTLISTED', 'INTERVIEW', 'OFFERED', 'REJECTED'].map((st) => (
          <button
            key={st}
              ...
              >
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#64748B] mt-0.5">
                        {app.company} • Applied on {app.appliedDate} • App ID: <span className="font-mono text-[#4F46E5]">{app.id}</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    {app.status === 'INTERVIEW' && (
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-[#4F46E5] hover:bg-indigo-700 text-white rounded-lg text-xs font-semibold shadow-xs transition-colors flex items-center gap-1"
                      >
                ...
                {app.status !== 'REJECTED' ? (
                  <div className="pt-2">
                    <div className="grid grid-cols-4 gap-2 text-center text-xs">
                      ...
                      <div className="space-y-1.5">
                        <div
                          className={`h-2 rounded-full ${
                            stage >= 3 ? 'bg-[#4F46E5]' : 'bg-slate-200'
                          }`}
                        />
                        <span className={`text-[11px] font-semibold ${stage >= 3 ? 'text-[#4F46E5]' : 'text-slate-400'}`}>
                          3. Technical Rounds
                        </span>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        <div
                          className={`h-2 rounded-full ${
                            stage >= 4 ? 'bg-[#22C55E]' : 'bg-slate-200'
                          }`}
                        />
                        <span className={`text-[11px] font-semibold ${stage >= 4 ? 'text-[#22C55E]' : 'text-slate-400'}`}>
                          4. Official Offer
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-xs text-rose-700 flex items-center gap-2">
                    ...
                    <span>Application closed. Candidate feedback: Strengthen DSA problem solving for future drives.</span>
                  </div>
                )}
              </div>
            );
